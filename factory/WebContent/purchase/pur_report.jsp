...
$(function(){
	...
	$('#btn_view').click(function(){
		if ($('#report_type').val() == '<%=StatusUtil.PUR_PO_LIST%>' || $('#report_type').val() == '<%=StatusUtil.INV_OUTLET%>') {
			if ($('#type_date').is(':checked')) {
				if ($('#date').val() != '') {
					popup('pur_report_review.jsp?' + form.serialize());
				} else {
	...
	$('#btn_export').click(function(){
		if ($('#report_type').val() == '<%=StatusUtil.INV_INLET%>' || $('#report_type').val() == '<%=StatusUtil.INV_OUTLET%>') {
			if ($('#type_date').is(':checked')) {
				if ($('#date').val() != '') {
					popup('pur_report_review.jsp?export=true&' + form.serialize());
				} else {
					alert('กรุณาระบุวันที่');
				}
			} else if ($('#type_month').is(':checked')) {
	...
	$('#report_type').change(function(){
		if ($(this).val() == '<%=StatusUtil.PUR_PO_LIST%>') {
			tr_type_time.show();
	...
	if ($('#report_type').val() == '<%=StatusUtil.PUR_PO_LIST%>') {
		tr_type_time.show();
		$('input:radio[name=time]').removeAttr('checked');
		tr_date.hide();
		tr_month.hide();
					...
					<tr>
						...
						<td>: 
							<bmp:ComboBox name="report_type" styleClass="txt_box s250" value="">
								<bmp:option value="<%=StatusUtil.PUR_PO_LIST%>" text="รายงานการสั่งซื้อ"></bmp:option>
							</bmp:ComboBox>
						</td>
					</tr>
