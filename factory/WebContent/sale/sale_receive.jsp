	...
	<div class="wrap_body">
		<div class="body_content">
			<div class="content_head">
				<div class="left">รายการคืน</div>
				<div class="right m_right10">
					<button class="btn_box btn_confirm" onclick="window.location='sale_select_inv.jsp'">สร้างใบรับคืน</button>			
				</div>
			</div>
			
			<div class="content_body">
			<form style="margin: 0; padding: 0;" action="sale_receive.jsp" id="search" method="get">
					...
					<table class="bg-image s930">
						...
						%>
							<tr>
								...
								<td align="center">
									<button class="btn_box btn_confirm" onclick="window.location='sale_add_receive.jsp?ch=1&order_id=<%=entity.getOrder_id()%>&pk_id=<%=entity.getPk_id()%>&FG=<%=entity.getFG()%>&item_type=<%=(entity.getPk_id().equalsIgnoreCase("")?"s":"p")%>&invoice=<%=entity.getInvoice()%>&type_inv=<%=entity.getType_inv()%>&id_receive=<%=entity.getId_receive()%>';">เลือก</button>	
								</td>
							</tr>
						<%
							}
			...
			<div class="wrap_desc">
				- หน้าจอนี้ใช้แสดงรายการรับสินค้าคืนจากลูกค้า 
			</div>
	</div>
	</div>
