...
<form id="comfirm_add_form" onsubmit="return false;">
	...
	<table width="100%"  class="center" id="tb_material_add" border="0" >
		<tbody>
			<tr>
				...
				<td align="left">
					<input type="radio" name="choose" value="0" checked="checked" onclick="$('#show_po').fadeOut('slow');" class="pointer"> ไม่มีใบ Po
			...
			<tr id="show_po" class="hide" >
				...
				<td align="left">
					<input type="text" class="txt_box" autocomplete="off" id="po" name="po" value="<%=entity.getPo()%>">
				</td>												
			</tr>
		</tbody>
	</table>
	...
	<div class="txt_center m_top20">
		...
		<input type="hidden" name="status" value="<%=SaleQt.STATUS_CLOSE_QT%>">	
		<input type="hidden" name="update_by" value="<%=securProfile.getPersonal().getPer_id()%>">
		<% if(order_type.equalsIgnoreCase(SaleOrder.TYPE_GHOST)) {%>			
		<input type="hidden" name="action" value="approve_qt_ghost">
		<%}else{ %>	
