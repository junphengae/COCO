...
$(function(){
	$('#btn_info').click(function(){
		...
		$('#request_dept').change(function(){
		ajax_load();
		$.post('../hr/EmpManageServlet',{dep_id: $(this).val(),action:'getPersonByDep'}, function(resData){
		ajax_remove();
		if (resData.status == 'success') {
			var options = '<option value="">--- เลือกแผนกผู้เบิก---</option>';
	        var data = resData.model;
	        $.each(data , function (index , object){
	        	options += '<option value="' + object[0] + '">' + object[1] + '</option>';
			});
	        $('#request_person').html(options);
		} else {
						...
						<legend>ข้อมูลการเบิก</legend>					
							<table width="100%">
								<tbody>
									...
									<tr>
										<td width="40%">แผนกผู้เบิก</td>
										<td width="60%">: 
											<bmp:ComboBox name="request_dept" listData="<%=Department.list()%>" styleClass="txt_box s150" validate="true" validateTxt="เลือกแผนกผู้เบิก!">
												<bmp:option value="" text="--- เลือกแผนกผู้เบิก---"></bmp:option>
											</bmp:ComboBox>	
										</td>
									</tr>
									<tr>
										<td width="40%">ผู้ขอเบิก</td>
										<td width="60%">: 
											<bmp:ComboBox name="request_person" styleClass="txt_box s150" validate="true" validateTxt="เลือกผู้เบิก!">
												<bmp:option value="" text="--- เลือกผู้เบิก---"></bmp:option>
											</bmp:ComboBox>	
										</td>
									</tr>
									<tr>
										<td width="30%">เลขอ้างอิงการเบิก</td>
										<td width="70%">: <%=WebUtils.getReqString(request,"request_id")%></td>
									</tr>
									<tr id="tr_lot_no">
										<td>Lot สินค้า</td>
							...
							<table width="100%" id="tb_input" class="hide">
								<tbody>
									<tr>
										...
										<td width="60%">: <span id="lot_balance"></span> <%=(invMaster.getGroup_id().equalsIgnoreCase("FG")?invMaster.getDes_unit():"KG")%></td>
									</tr>
									<tr><td colspan="2" height="20"></td></tr>
									<tr>
										<td>จำนวนที่เบิก</td>
										<td>: <input type="text" autocomplete="off" name="request_qty" id="request_qty" class="txt_box s150"> <%=(invMaster.getGroup_id().equalsIgnoreCase("FG")?invMaster.getDes_unit():"KG")%></td>
									</tr>
									<tr>
										<td>ผู้ทำรายการ </td>
										<td>: 
										<%=securProfile.getPersonal().getName()+" "+securProfile.getPersonal().getSurname()%>
											<input type="hidden" class="txt_box s150" id="request_by_autocomplete" value="<%=securProfile.getPersonal().getName()%>">
										</td>
									</tr>
									<tr><td colspan="2" height="10"></td></tr>
									<tr>
										<td colspan="2" align="center">
											<input type="hidden" name="request_no" value="<%=WebUtils.getReqString(request,"request_id")%>">
				...
				<fieldset class="s900 fset">
					...
					<table class="bg-image s900">
						...
						while (iteLot.hasNext()){
							...
							System.out.println("Lot_balance|"+lot_control.getLot_balance()+"|");
							if(!lot_control.getLot_balance().equalsIgnoreCase("")){
						...
						%>
							<tr>
								...
								<%}else{ %>
								<td colspan="5" align="right" class="txt_bold">ยอดที่สามารถเบิกได้: <%=Money.money(total)%> <%=invMaster.getStd_unit()%> [ <%=up%> <%=invMaster.getDes_unit()%> ]</td>
								<%} %>
							</tr>
						</tbody>
					</table>
