...
<title>ค้นหา Meterial(Search Meterial)</title>
...
session.setAttribute("PLAN_SEARCH", param);

if (WebUtils.getReqString(request, "action").length() == 0 && session.getAttribute("PLAN_SEARCH_PAGE") != null){
	ctrl.setPage_num(Integer.parseInt((String) session.getAttribute("PLAN_SEARCH_PAGE")));
}

if(page_.length() > 0){
	ctrl.setPage_num(Integer.parseInt(page_));
	session.setAttribute("PLAN_SEARCH_PAGE", page_);
}

List list = InventoryMaster.selectAllmaterial(ctrl, param);
%>
</head>
<body>
	...
	<div class="wrap_body">
		<div class="body_content">
			<div class="content_head">
				<div class="left">ค้นหา Meterial</div>
			...
			<div class="content_body">
				<div class="detail_wrap s900 center">
					<form style="margin: 0; padding: 0;" action="menu_search.jsp" id="search" method="get">
						<table>
							<tr>
								<td>ค้นหาจาก</td>
								<td>: 
								...
								<td>:
									<bmp:ComboBox name="group_id" styleClass="txt_box s120" listData="<%=Group.ddl_th()%>" value="<%=group_id%>">
								...
								<td>: 
									<bmp:ComboBox name="cat_id" styleClass="txt_box s120" value="<%=cat_id%>">
						...
						<div class="center s300 txt_center m_top5">
							<input type="submit" name="submit" value="ค้นหา" class="btn_box btn_confirm">
							<input type="button" name="reset" value="ล้าง" class="btn_box m_left5 btn_warn" onclick="resetSearch();">
				...
				<!-- next page  -->
				<div class="right txt_center m_right20"><%=PageControl.navigator_en(ctrl,"menu_search.jsp",param)%></div>
				<div class="clear"></div>
				<!-- next page  -->
				...
				<table class="bg-image s_auto">
					<thead>
						<tr>
							...
							<th valign="top" align="center" width="15%">สถานที่จัดเก็บ</th>
							<th width="10%"></th>
						</tr>
					</thead>
					<tbody>
					...
					%>
						<tr>
							...
							<td align="center">
								<button class="btn_view" onclick="window.location='material_view_notedit.jsp?mat_code=<%=entity.getMat_code()%>';"></button>
							</td>
						</tr>
					<%
					}
...
<script type="text/javascript">
	...
	$(function(){
		...
		if ($('#group_id').val() != '') {
			$.post('GetCat',{group_id:'<%=group_id%>',action:'get_cat_th'}, function(resData){
